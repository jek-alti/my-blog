...
4. **Embed** - 블로그 메인 페이지(index.html)에 웹앱 카드를 추가한다. 카드에는 제목, 설명, 미리보기 이미지 또는 iframe을 넣는다. 깃 커밋까지만 한다.
...
- 모바일에서도 사용할 수 있어야 한다.
- 모든 웹앱은 블로그의 색상 팔레트를 따른다. 색상은 하드코딩하지 말고 style.css의 CSS 변수(예: `--bg`, `--text`, `--accent` 등)를 참조한다. (앱 HTML에서 블로그 style.css를 링크하거나 동일한 변수를 정의해 사용)
- 웹앱에 사용법 안내 문구를 반드시 포함한다. (조작 방법·기능을 화면에서 바로 알 수 있도록)
